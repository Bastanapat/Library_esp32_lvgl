...
void setup() {
  ...
  // Create a button
  lv_obj_t *ui_STBT = lv_btn_create(lv_scr_act());
  lv_obj_set_width(ui_STBT, 102);
  lv_obj_set_height(ui_STBT, 102);
  lv_obj_set_x(ui_STBT, 0);
  lv_obj_set_y(ui_STBT, 0);
  lv_obj_set_align(ui_STBT, LV_ALIGN_CENTER);
  lv_obj_add_flag(ui_STBT, LV_OBJ_FLAG_SCROLL_ON_FOCUS);  /// Flags
  lv_obj_clear_flag(ui_STBT, LV_OBJ_FLAG_SCROLLABLE);     /// Flags
  lv_obj_set_style_bg_color(ui_STBT, lv_color_hex(0x5B6FF4), LV_PART_MAIN | LV_STATE_DEFAULT);
  lv_obj_set_style_bg_opa(ui_STBT, 255, LV_PART_MAIN | LV_STATE_DEFAULT);
  lv_obj_add_event_cb(ui_STBT, event_STBT, LV_EVENT_ALL, NULL);

  lv_obj_t *ui_Label1 = lv_label_create(ui_STBT);
  lv_obj_set_width(ui_Label1, LV_SIZE_CONTENT);   /// 1
  lv_obj_set_height(ui_Label1, LV_SIZE_CONTENT);  /// 1
  lv_obj_set_align(ui_Label1, LV_ALIGN_CENTER);
  lv_label_set_text(ui_Label1, "ON/OFF");
